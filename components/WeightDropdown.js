            ...
            onSelect={obj => {
              if (props.availableDropdowns.includes(x)) {
                if (selection != 0) {
                  // readd item
                  console.log(
                    `re-add ${selection} to the props.availableDropdowns`
                  );
                ...
                if (x != 0) {
                  props.availableDropdowns.splice(
                    props.availableDropdowns.indexOf(x),
                    1
                  );
                  console.log(
                    `props.availableDropdowns.includes(x): ${props.availableDropdowns}`
                  );
                }
                props.setAvailableDropdowns(props.availableDropdowns);
                // don't trigger the update if both x and selection are 0
                if (x != 0 || selection != 0) {
                  props.onSelectSelection();
                }
              } else {
                alert(
